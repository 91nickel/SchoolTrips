...
if($an['auth']==1)
{
    ...
    foreach ($cd as $key => $value) {
    	if(count($value)<=5)
    	{
    		...
    		foreach ($value as $key2 => $value2) {
    			if(count($first)==0)
    				$first = $contacts_id[$value2];
    			if($first['date_create']>$contacts_id[$value2]['date_create'])
    				$first = $contacts_id[$value2];
    		}
    		...
    		foreach ($last['custom_fields'] as $key2 => $value2) {
    			if(($value2['id']==95404)||($value2['id']==95406))
    				continue;
    			$up['cfv'][$value2['id']] = $value2['values'][0]['value'];
    		}
    		foreach ($last['custom_fields'] as $key2 => $value2) {
    			if(($value2['id']==95404)||($value2['id']==95406))
    			{
    				$up['cfv'][$value2['id']] = [json_encode(["DESCRIPTION"=>"WORK","VALUE"=>$value2['values'][0]['value']])];
    			}
    		}
    		foreach ($value as $key2 => $value2) {
    			if(isset($contacts_id[$value2]['linked_company_id']))
    				$up['COMPANY_UID'] = $contacts_id[$value2]['linked_company_id'];
    		}
    		$up['MAIN_USER_ID'] = $last['responsible_user_id'];
	    	$leads_id = [];	
	    	foreach ($value as $key2 => $value2) {
    		...
    		$up['TAGS'] = [];
    		foreach ($value as $key2 => $value2) {
    			foreach ($contacts_id[$value2]['tags'] as $key3 => $value3) {
    				$up['TAGS'][]=$value3['id'];
    			}
    		}
    		...
    		echo 'merge <br>'.json_encode(["id"=>$ids,"result_element"=>$up])."<br><br>";

    		meth2('/ajax/contacts/merge/save',["id"=>$ids,"result_element"=>$up]);
    	}
    }
}
