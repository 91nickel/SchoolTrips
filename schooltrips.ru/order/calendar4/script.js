...
var abon = {};
  ...
  function showList(data)
  {
    ...
    for(var i in data)
    {
      ...
      for(var t in data[i].custom_fields)
      {
        ...
        if(data[i].custom_fields[t].id==578909)
          places_all_adult = data[i].custom_fields[t].values[0].value;
        if(data[i].custom_fields[t].id==594140)
          folder = data[i].custom_fields[t].values[0].value;
      }

      if((otdel!='')&&(otdel!='Основной'))
        continue;
      ...
      event.place = place;
      event.folder = folder;
      event.places_all_adult = places_all_adult;
      event.type = type;
    ...
    for(var i in events)
    {
      ...
      {
        if(events[i].title.indexOf('майлово')!=-1)
          events[i].place = 'Измайловский кремль';
        else 
          events[i].place = '';
        if(events[i].type_s=='Сборная')
          $('.events.main .block').append('<div json=\''+json+'\' class="event mini '+classes+'"><div class="time">'+events[i].start+'</div><div class="places">'+(events[i].places_ost>5?'более 5':events[i].places_ost)+' мест свободно'+'</div><div class="places2">'+events[i].places+' мест всего'+'</div><div class="place">'+events[i].place+'</div>'+(events[i].old==' лет'?'':'<div class="old">'+events[i].old+'</div>')+'<div class="price">'+events[i].price+'р</div>'+'<div class="btn">Открыть</div></div>');
        else
        ...
        $('.events.main .block').append('<div json=\''+json+'\' class="event '+classes+'"><div class="title">'+events[i].title+'</div><div class="places">'+(events[i].places_ost>5?'более 5':events[i].places_ost)+' мест свободно'+'</div><div class="places2">'+events[i].places+' мест всего'+'</div><div class="time">'+events[i].start+'</div>'+(events[i].old==' лет'?'':'<div class="old">'+events[i].old+'</div>')+'<div class="price">'+events[i].price+'р</div><a target="_blank" href="/event/'+events[i].folder+'" class="btn more">Подробнее</a><div class="btn">Открыть</div></div>');
    }
    $('.events.main .event .btn').click(function(){
      ...
      if(user.name!=undefined)
      {
        ...
        $('#form #tel').val(user.phone);
        $('#form #name, #form #email').hide();
        $('#form #tel').attr('disabled',true);
      }else{
        $('#form #name, #form #email').show();
        $('#form #tel').attr('disabled',false);
      }

      event_opened = list[calEvent.className.substr(2)];
      cal_event_opened = calEvent;

      if(cal_event_opened.places_all_adult==0)
      {
        $('#p2').hide();
        if($('#p2').parent().find('.info').length==0)
          $('#p2').parent().append('<div class="uk-alert-danger info" uk-alert><p>ВАЖНО! Билеты для взрослых приобретайте самостоятельно в кассе музея</p></div>');
      }else{
        $('#p2').show();
        $('#p2').parent().find('.info').remove();
      }
  ...
  $(document).ready(function() {
    ...
    if(window.location.href.indexOf('amo')!=-1)
    $('a').each(function(){
    ...
    if($('.events_exc').length>0)
    {
      ...
      $.get('../get_leads.php',{tel: user.phone}, function(data){
        console.log('m2');
        $.get('../get_abon.php',{con:user.id},function(abon){
          console.log('m3');
          $('.events_exc .block').html('');
          if(data!='')
          {
            console.log('m4');
            data = $.parseJSON(data);
            ...
            for(var i in data){
              ...
              if(data[i].status_id==15562216)
                t1+=('<div class="event"><div class="title">'+title+'</div><div class="time">'+start+'</div><div class="cur">Состав: '+kids+' детей + '+adult+' взрослых</div>'+'<div class="price">'+price+'р</div><a href="https://schooltrips.ru/rfibank/url.php?id='+data[i].id+'" target="_blank" class="uk-button pay uk-button-primary">Оплатить</a></div>');
            ...
            console.log('T2');
            if(abon.abons)
              for(var t in abon.abons)
                t4+=('<div class="event" style="min-height: auto;padding-bottom: 10px;"><div class="title">Абонемент</div><div class="time">Осталось: '+abon.abons[t].c+'</div><div class="cur">Всего экскурсий: '+abon.abons[t].all+'</div></div>');
            if(t1!='')
            {
            ...
            if(t4!='')
            {
              $('.events_exc .block').append('<div class="date"><div class="t">Абонементы</div></div>');
              $('.events_exc .block').append(t4);
            }
          }
        },'json');
      });
    ...
    $('button[name=auth]').click(function(){
      ...
      tel = tel.split('(').join('');
      tel = tel.split(')').join('');
      window.location.href = 'https://schooltrips.ru/?link=exc.php?userphone=+'+tel;
    })
    ...
    $('.makeOrder').click(function(e){
      ...
      if(((event_opened.k1=='')||(event_opened.k1==0)||(event_opened.name=='')||(event_opened.tel==''))&&(window.location.href.indexOf('amo')==-1))
      {
        UIkit.modal.alert('Необходимо заполнить все данные');
        return false;
      }
    ...
    $('.makeOrderAbon').click(function(e){
      ...
      if((event_opened.p2==0)||(event_opened.p2==''))
        event_opened.sale = 1;
      event_opened.cat_id = event_opened.id;
      event_opened.con = user.id;
      event_opened.abon = abon_id;
      UIkit.modal.dialog('<div style="padding: 20px;text-align: center;">Формирование заказа</div>');
      $.post('../../excursions/newAbon.php',event_opened,function(data){
        var text = 'Спасибо, за день до экскурсии(до 18:00) вы получите детали с местом встречи и телефоном куратора';
        if(event_opened.sale!=1)
          text = "Спасибо! Вам на телефон направлено смс для оплаты билетов для взрослых, т.к. они не входят в абонемент. После оплаты вы получите ссылку на билет по смс.";
