...
function toamo($ob)
{
	...
	$toamo = 'http://nova-agency.ru/widgets/api/site_integration/new.php?'.http_build_query(['name'=>$ob['name'],'phone'=>$ob['phone'],'email'=>$ob['Email'],'roistat'=>$_COOKIE['roistat_visit'],'subdomain'=>'spartaequip','url'=>$actual_link]);
	file_put_contents('form.log', $toamo.chr(10).chr(10), FILE_APPEND);
    file_get_contents($toamo);
}
...
function email_form_submission($form) {
	...
	$sent = @mail('info@orrsowear.com', $subject, $message, $headers);
...
function get_email_body($subject, $heading, $fields, $resources) {
	...
	{
		$ob[$field_wrapper['field']] = get_form_field_value($field_wrapper['field'], $field_wrapper['properties'], $resources, true);
