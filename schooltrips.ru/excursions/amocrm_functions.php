...
function getListCatalog($account, $catalogId, $offset){
    $request = array(
...
function updateListEl($account, $list){
    $link='https://'.$account['amocrm_domain'].'.amocrm.ru/api/v2/catalog_elements';
    $curl=curl_init(); #Сохраняем дескриптор сеанса cURL
#Устанавливаем необходимые опции для сеанса cURL
    curl_setopt($curl,CURLOPT_RETURNTRANSFER,true);
    curl_setopt($curl,CURLOPT_USERAGENT,'amoCRM-API-client/1.0');
    ...
    curl_setopt($curl,CURLOPT_POSTFIELDS,json_encode($list));
...
function link_catalog_amoCRM($account,$data){
  $link='https://'.$account['amocrm_domain'].'.amocrm.ru/private/api/v2/json/links/set';
    ...
    {
      die('Ошибка link_catalog_amoCRM: '.$E->getMessage().PHP_EOL.'Код ошибки: '.$E->getCode());
    }
    $Response=json_decode($out,true);
     //print_r($Response);
    return $Response;
...
function create_contact_amoCRM($account,$data){
...
function create_lead_amoCRM($account,$data){
  $link='https://'.$account['amocrm_domain'].'.amocrm.ru/api/v2/leads';
