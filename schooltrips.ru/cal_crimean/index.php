...
<body>
	<iframe src="http://nova-agency.ru/auto/schooltrip/order/calendar4_crimea/<?=isset($_GET['link'])?($_GET['link']):'#exc='.$_GET['exc']?>" frameborder="0"></iframe>
		...
		window.addEventListener('message', function(event) {
			...
			if(event.data.name=='order')
			{
				...
				$.get('http://nova-agency.ru/auto/schooltrip/excursions/new.php',data,function(data){
					console.log(data);
					$('.text').html('Спасибо. Ваш заказ сформирован. Вам на почту должно прийти письмо с дальнейшими деталями.');
